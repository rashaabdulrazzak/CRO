...
import { getMenubarItems } from "../config/menuConfig";
const NavBar = () => {
  ...
  const availableMenuItems = user?.role
    ? getMenubarItems(user.role, navigate)
    : [];
  const items = [
    {
      label: "Rasha",
...
{/*       <i className="pi pi-globe text-lg cursor-pointer"></i>
  ...
  return (
    <div className="card ">
      ...
      <Menubar
        className="Navbar px-3 py-1.5"
        model={availableMenuItems}
        start={start}
        end={end}
      />
    </div>
