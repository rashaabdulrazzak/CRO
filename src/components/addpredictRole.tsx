...
// ================== ROLES & ACCESS ==================
export type Role = "radiologist" | "field_coordinator" | "monitor" | "Hekim" | "Koordinatör";

const steps = [
  { label: "Demographic Information" }, // 0
  { label: "Medical History" },         // 1
  { label: "Inclusion Criteria" },      // 2
  { label: "Exclusion Criteria" },      // 3
  { label: "AI Diagnostics" },          // 4
  { label: "Upload Photo" },            // 5
];

// who can view/edit each step
const STEP_ACCESS: Array<{ canView: Role[]; canEdit: Role[] }> = [
  { canView: ["radiologist" , "field_coordinator","monitor"], canEdit: ["field_coordinator","monitor"] }, // 0
  { canView: ["radiologist" ,"field_coordinator","monitor"], canEdit: ["field_coordinator","monitor"] }, // 1
  { canView: ["radiologist" ,"field_coordinator","monitor"], canEdit: ["field_coordinator","monitor"] }, // 2
  { canView: ["radiologist" ,"field_coordinator","monitor"], canEdit: ["field_coordinator","monitor"] }, // 3
  { canView: ["radiologist" ,"field_coordinator","monitor"], canEdit: ["radiologist","monitor"] },       // 4
  { canView: ["radiologist" ,"field_coordinator","monitor"], canEdit: ["field_coordinator","monitor"] }, // 5
];

const canViewStep   = (i: number, role: Role) => STEP_ACCESS[i].canView.includes(role);
const canEditStep   = (i: number, role: Role) => STEP_ACCESS[i].canEdit.includes(role);
...
export const AddNewPredictDialog: React.FC<AddNewPredictDialogProps> = ({
  ...
  // Ensure current step is viewable (with our policy it always is, but safe-guard)
  useEffect(() => {
    if (!canViewStep(activeIndex, currentRole)) {
      // find the first viewable step
      const first = steps.findIndex((_, i) => canViewStep(i, currentRole));
      setActiveIndex(first >= 0 ? first : 0);
    }
  }, [activeIndex, currentRole]);
  ...
  }, [activeIndex, demographic, medical, inclusion, exclusion, ai, uploadedPhotos, patientId, caseId]);
  ...
  // ================== TOAST ==================
  const showError   = (detail: string) => toast.current?.show({ severity: "error",   summary: "Hata",     detail, life: 3000 });
  const showInfo    = (detail: string) => toast.current?.show({ severity: "info",    summary: "Bilgi",    detail, life: 2000 });
  const showSuccess = (detail: string) => toast.current?.show({ severity: "success", summary: "Başarılı", detail, life: 2500 });
  ...
  const handleSaveAndNext = async () => {
    const readOnly = !canEditStep(activeIndex, currentRole);
    const atLastStep = activeIndex === steps.length - 1;

    // If this step is read-only for the current role, just move on / close.
    if (readOnly) {
      if (atLastStep) {
        onHide();
      } else {
    ...
    try {
      // ---- Editable step logic
      if (activeIndex === 0) {
        // Create patient with validations (Koordinatör only)
      ...
      if (activeIndex === 2) {
        ...
        if (!allTrue) {
          showError("Dahil etme kriterlerinin tamamı 'Evet' olmalıdır. Devam edilemez.");
          return;
        }
      }

      if (activeIndex === 3) {
        // Exclusion: if ANY true -> block
        const anyTrue = Object.values(exclusion).some((v) => v === true);
        if (anyTrue) {
      ...
      // FINALIZE (only Koordinatör, last step)
      if (atLastStep) {
        if (currentRole !== "Koordinatör") {
          // Hekim cannot finalize the bundle
          onHide();
          return;
        }
        ...
        const visitPayload = {
          ...
          secondVisitDate: demographic.secondVisitDate ? demographic.secondVisitDate.toISOString().split("T")[0] : null,
        ...
        // Eligibility (medical + inclusion + exclusion)
        ...
        // AI form with TIRADS (Hekim should have set these on step 4)
  ...
  const renderStepContent = () => {
    switch (activeIndex) {
      case 0: return renderDemographicInformation();
      case 1: return renderMedicalHistory();
      case 2: return renderInclusionCriteria();
      case 3: return renderExclusionCriteria();
      case 4: return renderAIDiagnostics();
      case 5: return renderUploadPhoto();
      default: return <div className="text-center py-8 text-gray-500">Step content coming soon...</div>;
    }
  };
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <InputText
              ...
              onChange={(e) => setDemographic((p) => ({ ...p, volunteerCode: e.target.value }))}
              onBlur={(e) => setDemographic((p) => ({ ...p, volunteerCode: normalizeVolunteerCode(e.target.value) }))}
        ...
        <div>
          ...
          <InputText value={demographic.protocolNo} onChange={(e) => setDemographic((p) => ({ ...p, protocolNo: e.target.value }))} className="w-full" />
        </div>

        {/* First Visit Date */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">First Visit Date (USG)</label>
          <Calendar value={demographic.visitDate} onChange={(e) => setDemographic((p) => ({ ...p, visitDate: (e.value as Date) ?? null }))} className="w-full" inputClassName="w-full" showIcon />
        </div>

        {/* Name Surname */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Name Surname</label>
          <InputText value={demographic.nameSurname} onChange={(e) => setDemographic((p) => ({ ...p, nameSurname: e.target.value }))} placeholder="Name Surname" className="w-full" />
        </div>

        {/* Birth Date */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Birth Date</label>
          <Calendar value={demographic.birthDate} onChange={(e) => setDemographic((p) => ({ ...p, birthDate: (e.value as Date) ?? null }))} className="w-full" inputClassName="w-full" showIcon />
        </div>

        {/* Second Visit Date */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Second Visit Date (Pathology)</label>
          <Calendar value={demographic.secondVisitDate} onChange={(e) => setDemographic((p) => ({ ...p, secondVisitDate: (e.value as Date) ?? null }))} className="w-full" inputClassName="w-full" showIcon />
        </div>

        {/* Body Mass Index */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Body Mass Index</label>
          <InputText value={demographic.bodyMassIndex} onChange={(e) => setDemographic((p) => ({ ...p, bodyMassIndex: e.target.value }))} className="w-full" />
        </div>

        {/* USG Device / Probe */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">USG Device / Probe</label>
          <InputText value={demographic.usgDevice} onChange={(e) => setDemographic((p) => ({ ...p, usgDevice: e.target.value }))} placeholder="e.g., Brand Model, Probe MHz" className="w-full" />
        </div>

        {/* Referred From */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Referred From</label>
          <Dropdown value={demographic.referredFrom} options={[
          ...
          ]} onChange={(e) => setDemographic((p) => ({ ...p, referredFrom: e.value }))} className="w-full" panelClassName="mt-1" />
        </div>

        {/* Weight */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Weight (kg)</label>
          <InputText value={demographic.weight} onChange={(e) => setDemographic((p) => ({ ...p, weight: e.target.value }))} className="w-full" />
        </div>

        {/* Size */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Size (cm)</label>
          <InputText value={demographic.size} onChange={(e) => setDemographic((p) => ({ ...p, size: e.target.value }))} className="w-full" />
        </div>
            ...
            {(["male", "female", "not-specify"] as Sex[]).map((val) => (
              <div className="flex items-center" key={val}>
                <RadioButton
                  ...
                  name="gender"
                  value={val}
                  onChange={(e) => setDemographic((p) => ({ ...p, gender: e.value }))}
    ...
    <div className="space-y-6">
          ...
          {(["yes", "no"] as YesNo[]).map((val) => (
            <div className="flex items-center" key={val}>
              <RadioButton
                ...
                value={val}
                onChange={(e) => setMedical((p) => ({ ...p, medicalQuestion1: e.value }))}
      ...
      <div>
        ...
        <Dropdown
          ...
          onChange={(e) => setMedical((p) => ({ ...p, howManyYears: e.value }))}
          className="w-48"
          panelClassName="mt-1"
        />
      </div>
            ...
            {(["yes", "no"] as YesNo[]).map((val) => (
              <div className="flex items-center" key={val}>
                <RadioButton
                  ...
                  value={val}
                  onChange={(e) => setMedical((p) => ({ ...p, [`medicalQuestion${n}`]: e.value as YesNo }))}
      ...
      <div>
        ...
        <InputText
          ...
          onChange={(e) => setMedical((p) => ({ ...p, diseaseType: e.target.value }))}
          ...
          {[
            ...
            <div className="flex items-center gap-3" key={key}>
              <Checkbox
                ...
                onChange={(e) => setAi((prev) => ({ ...prev, composition: { ...prev.composition, [key]: e.checked } }))}
              />
          ...
          {[
            ...
            <div className="flex items-center gap-3" key={key}>
              <Checkbox
                ...
                onChange={(e) => setAi((prev) => ({ ...prev, echogenicity: { ...prev.echogenicity, [key]: e.checked } }))}
              />
          ...
          {[
            ...
            <div className="flex items-center gap-3" key={key}>
              <Checkbox
                ...
                onChange={(e) => setAi((prev) => ({ ...prev, shape: { ...prev.shape, [key]: e.checked } }))}
              />
          ...
          {[
            ...
            <div className="flex items-center gap-3" key={key}>
              <Checkbox
                ...
                onChange={(e) => setAi((prev) => ({ ...prev, margin: { ...prev.margin, [key]: e.checked } }))}
              />
          ...
          {[
            ...
            <div className="flex items-center gap-3" key={key}>
              <Checkbox
                ...
                onChange={(e) => setAi((prev) => ({ ...prev, echogenicFocii: { ...prev.echogenicFocii, [key]: e.checked } }))}
              />
  ...
  const renderUploadPhoto = () => {
    ...
    const readOnly = !canEditStep(5, currentRole); // explicit disable for a11y on this widget
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex gap-4 flex-wrap">
          ...
          <div className={`w-32 h-32 border-2 border-dashed rounded-lg flex flex-col items-center justify-center ${readOnly ? "bg-gray-100 opacity-60 cursor-not-allowed" : "bg-gray-50 hover:bg-gray-100 cursor-pointer"}`}>
            <FileUpload
              mode="basic"
              name="photo"
  ...
  const primaryLabel = atLastStep
    ? (currentRole === "Koordinatör" ? "Finish" : "Close")
    : (readOnly ? "Next" : "Save and Next");

  const headerElement = (
    <div className="flex items-center gap-3">
      <button onClick={handleDialogClose} className="text-gray-500 hover:text-gray-700 text-xl">×</button>
      <span className="text-lg font-medium text-gray-800">Add New Predict</span>
      <span className="ml-auto text-xs rounded-full px-2 py-1 bg-gray-100 text-gray-800 border">Role: {currentRole}</span>
    </div>
  );
      ...
      <div className="p-6">
        ...
        {readOnly && (
          <div className="mb-3 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
            Bu adım <b>{currentRole}</b> için <b>salt okunur</b>.
          </div>
        )}
        ...
        <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-200">
          <Button label="Cancel" onClick={handleDialogClose} className="px-6 py-2 text-gray-600 bg-white border border-gray-300 rounded-md hover:bg-gray-50" text />
          <Button label={primaryLabel} onClick={handleSaveAndNext} className="px-6 py-2 rounded-md p-button-secondary" />
        </div>
      </div>
    </Dialog>
  );
